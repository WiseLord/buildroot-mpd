...
class Player(object):
    ...
    def handle_autoplay(self):
        if self.autoplay_handled:
            return
        state = self.client.status().get('state')
        if state == 'stop':
            song = self.client.currentsong()
            if (not song.get('file', '').startswith('http://') or
                self.networkChecker.get_ip() != '127.0.0.1'):
                self.client.play()
                self.autolay_handled = True
    ...
    def send_ip(self):
        self.console.send('ip:' + self.player_info['ip'])
